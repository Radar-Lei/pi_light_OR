...
from lock_v15_r68_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r68_reactivated_dual_native_double_horizon_blend_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_double_blend"] == 0.25
    assert protocol["controller_params"]["route_horizon_native_score_blend"] == 1.25
    assert protocol["controller_params"]["route_horizon_pressure_blend"] == 0.10


def test_r68_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r68_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r68_route_horizon_blend_interpolates_between_native_and_double(monkeypatch) -> None:
    ...
    def fake_dynamic_phase_decomposition(
        ...
        *,
        ...
        total = 10.0 if phase_index == 0 else 100.0
        component_totals = {
            "cascade_price": 0.0,
    ...
    monkeypatch.setattr(
        ...
        lambda phase_idx, *args, **kwargs: 100.0 if phase_idx == 0 else 95.0,
    )
