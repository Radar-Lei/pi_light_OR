...
from lock_v15_r89_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r89_reactivated_dual_low_signal_margin_cap_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_low_signal_margin_epsilon"] == 1e-6


def test_r89_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r89_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r89_only_caps_thin_margin_low_signal_outrank(monkeypatch) -> None:
    ...
    r89_audit = select_finite_storage_action_with_audit(
        "J0",
        0,
        phase_states,
        movements,
    ...
    assert r89_audit["route_horizon_completion_filter_used"] is True
    assert r89_audit["route_horizon_negative_total_completion_conflict_guard_used"] is False
    assert r89_audit["route_horizon_blended_phase_details"]["1"]["thin_margin_low_signal_capped"] == 1.0
    assert r89_audit["route_horizon_blended_phase_details"]["1"]["pre_cap_blended_score"] == 2.0
    assert r89_audit["selected_action"] == 0
