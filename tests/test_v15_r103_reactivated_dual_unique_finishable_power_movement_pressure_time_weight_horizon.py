...
from lock_v15_r103_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r103_reactivated_dual_unique_finishable_power_movement_pressure_time_weight_horizon_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_conditional_phase_time_weight_movement_downstream_pressure_floor"] == 1.0


def test_r103_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r103_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r103_conditional_time_weight_only_hits_risky_movements_inside_supported_phase() -> None:
    states = ["GG"]
    movements = [
        ("A", "B"),
        ("C", "D"),
    ...
    finite_storage_state = {
        "residual_receiving_capacity": {"B": 1.5, "D": 6.0},
    ...
    assert details["C->D"]["effective_time_weight_power"] == 1.0
