...
from lock_v15_r77_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r77_reactivated_dual_conditional_risk_blend_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_conditional_risk_end_fraction"] == 0.25
    assert protocol["controller_params"]["route_horizon_conditional_risk_occupancy_threshold"] == 1.05
    assert protocol["controller_params"]["route_horizon_conditional_risk_residual_threshold"] == 0.0


def test_r77_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r77_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r77_negative_risk_penalties_only_apply_in_early_severe_regime(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda phase_idx, *args, **kwargs: 100.0 if phase_idx == 0 else 98.0,
    )
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: {"max_occupancy_ratio": 1.2, "min_residual_ratio": 0.0},
    )

    early_audit = select_finite_storage_action_with_audit(
        "J0",
        0,
        phase_states,
        movements,
    ...
    late_audit = select_finite_storage_action_with_audit(
        "J0",
        0,
        phase_states,
        movements,
    ...
    assert early_audit["route_horizon_completion_filter_used"] is True
    assert early_audit["selected_action"] == 1
    assert late_audit["route_horizon_completion_filter_used"] is True
    assert late_audit["selected_action"] == 0
