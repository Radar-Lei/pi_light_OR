...
from lock_v15_r92_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r92_reactivated_dual_supported_score_gap_cap_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_low_signal_margin_cap"] == 0.15
    assert protocol["controller_params"]["route_horizon_low_signal_margin_core_horizon_floor"] == 0.5
    assert protocol["controller_params"]["route_horizon_low_signal_margin_core_score_advantage_floor"] == 2.0


def test_r92_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r92_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r92_caps_only_when_core_baseline_has_large_raw_score_gap(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda phase_idx, *args, **kwargs: {
            "score": 10.0 if phase_idx == 0 else 1.0,
            "components": {
                "finishable_term": 10.0 if phase_idx == 0 else 1.0,
    ...
    assert r92_audit["route_horizon_blended_phase_details"]["1"]["thin_margin_low_signal_capped"] == 1.0
    assert r92_audit["route_horizon_blended_phase_details"]["1"]["route_horizon_score_gap_to_best_core"] == 9.0
    assert r92_audit["selected_action"] == 0


def test_r92_skips_cap_when_raw_score_gap_is_small(monkeypatch) -> None:
    phase_states, movements, queues, capacities, state, dual_state, route_completion_state = base_inputs()
    ...
    monkeypatch.setattr(
        ...
        "phase_route_horizon_completion_decomposition",
        lambda phase_idx, *args, **kwargs: {
            "score": 2.5 if phase_idx == 0 else 1.0,
            "components": {
                "finishable_term": 2.5 if phase_idx == 0 else 1.0,
                "downstream_penalty": 0.0,
                "local_pressure_penalty": 0.0,
            },
            "movement_details": [],
    ...
    assert r92_audit["route_horizon_blended_phase_details"]["0"]["horizon_term"] == 1.0
    assert "thin_margin_low_signal_capped" not in r92_audit["route_horizon_blended_phase_details"]["1"]
    assert r92_audit["route_horizon_capped_phases"] == []
