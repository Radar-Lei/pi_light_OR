...
from lock_v15_r36_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r36_late_deadline_anchor_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_deadline_base_weight"] == 0.25


def test_r36_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r36_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r36_deadline_horizon_filter_is_gated_until_late_window() -> None:
    phase_states, movements, queues, capacities, state, dual_state, route_completion_state = base_inputs()

    before_late_window = select_finite_storage_action_with_audit(
        "J0",
        0,
        phase_states,
        movements,
    ...
    late_window = select_finite_storage_action_with_audit(
        "J0",
        0,
        phase_states,
        movements,
    ...
    assert before_late_window["route_horizon_completion_filter_used"] is False
    assert late_window["route_horizon_completion_filter_used"] is True
