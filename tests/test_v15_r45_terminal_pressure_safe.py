...
from lock_v15_r45_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r45_preterminal_pressure_safe_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_pressure_safe_start_fraction"] == 0.70
    assert "route_horizon_deadline_time_urgency" not in protocol["controller_params"]


def test_r45_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r45_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r45_pressure_safe_guard_is_preterminal_gated() -> None:
    ...
    preterminal = select_finite_storage_action_with_audit(
        "J0",
        0,
        phase_states,
        movements,
    ...
    assert early["route_horizon_completion_filter_used"] is True
    assert early["route_horizon_pressure_safe_guard_used"] is False
    assert preterminal["route_horizon_completion_filter_used"] is True
    assert preterminal["route_horizon_pressure_safe_guard_used"] is True
    assert preterminal["selected_action"] == preterminal["pressure_action"]
