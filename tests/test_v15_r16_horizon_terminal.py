...
from lock_v15_r16_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def _state(queues: dict[str, float]) -> dict[str, object]:
    return {
...
def test_r16_horizon_terminal_controller_is_registered() -> None:
...
def test_r16_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r16_training_only" for row in spec)
...
def test_r16_terminal_lock_prevents_late_horizon_override() -> None:
