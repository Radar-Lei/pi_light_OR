...
from lock_v15_r58_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r58_reactivated_dual_post_veto_double_safe_controller_is_registered() -> None:
...
def test_r58_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r58_training_only" for row in spec)
