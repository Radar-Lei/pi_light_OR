...
from lock_v15_r99_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r99_reactivated_dual_unique_finishable_ratio_horizon_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_total_demand_power"] == 1.0


def test_r99_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r99_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_route_horizon_total_demand_power_normalizes_raw_completion_volume() -> None:
    states = ["GG"]
    movements = [
        ("A", "B"),
        ("C", "D"),
    ...
    assert normalized_details["A->B"]["effective_total_demand"] == 4.0
    assert normalized_details["A->B"]["finishable_term"] == 0.8
    assert normalized_details["C->D"]["finishable_term"] == 0.8
