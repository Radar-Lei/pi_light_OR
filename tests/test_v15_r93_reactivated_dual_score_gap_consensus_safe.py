...
from lock_v15_r93_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r93_reactivated_dual_score_gap_consensus_safe_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_score_gap_consensus_guard"] == 1.0
    assert protocol["controller_params"]["route_horizon_score_gap_consensus_total_ceiling"] == -15.0
    assert protocol["controller_params"]["route_horizon_score_gap_consensus_gap_floor"] == 2.0


def test_r93_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r93_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r93_noncompletion_score_gap_consensus_guard_only_catches_severe_negative_total(monkeypatch) -> None:
    ...
    monkeypatch.setattr(sumo, "completion_risk_active", lambda *args, **kwargs: False)
    monkeypatch.setattr(
        sumo,
    ...
    r93_audit = select_finite_storage_action_with_audit(
        "J0",
        0,
        phase_states,
        movements,
    ...
    assert r92_audit["route_horizon_completion_filter_used"] is False
    assert r92_audit["selected_action"] == 1
    assert r93_audit["route_horizon_completion_filter_used"] is False
    assert r93_audit["route_horizon_score_gap_consensus_guard_used"] is True
    assert r93_audit["selected_action"] == 0
