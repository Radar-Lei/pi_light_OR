...
from lock_v15_r72_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r72_reactivated_dual_release_risk_horizon_blend_controller_is_registered() -> None:
...
def test_r72_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r72_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r72_route_horizon_blend_prefers_release_supported_low_risk_phase(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda phase_idx, *args, **kwargs: 100.0 if phase_idx == 0 else 96.0,
    )
