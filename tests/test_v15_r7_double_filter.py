...
from lock_v15_r7_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r7_double_filter_controller_is_registered() -> None:
...
def test_r7_training_protocol_uses_fresh_seeds_and_is_not_claim_ready() -> None:
    ...
    assert set(split["seeds"]).isdisjoint(R6_TRAINING_SEEDS)
    assert protocol["claim_scope"]["closed_loop_superiority_claim_allowed"] is False


def test_r7_filters_dynamic_action_when_double_score_is_lower() -> None:
    ...
    assert audit["double_score_safety_filter_used"] is True
