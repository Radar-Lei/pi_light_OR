...
from lock_v15_r51_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r51_core_consensus_safe_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_core_consensus_guard"] == 1.0
    assert protocol["controller_params"]["route_horizon_core_consensus_start_fraction"] == 0.0


def test_r51_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r51_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r51_core_consensus_guard_gate_opens_immediately_after_warmup() -> None:
