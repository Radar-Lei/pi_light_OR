...
from lock_v15_r88_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r88_reactivated_dual_low_signal_bonus_scale_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_low_signal_bonus_scale"] == 0.9


def test_r88_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r88_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r88_low_signal_bonus_scale_only_shaves_weak_bonus_margin(monkeypatch) -> None:
    ...
    r88_audit = select_finite_storage_action_with_audit(
        "J0",
        0,
        phase_states,
        movements,
    ...
    assert r88_audit["route_horizon_completion_filter_used"] is True
    assert r88_audit["route_horizon_negative_total_completion_conflict_guard_used"] is False
    assert r88_audit["route_horizon_blended_phase_details"]["1"]["low_signal_bonus_blocked"] == 1.0
    assert r88_audit["route_horizon_blended_phase_details"]["1"]["low_signal_bonus_scale"] == 0.9
    assert r88_audit["route_horizon_blended_phase_details"]["1"]["release_term"] == 0.9
    assert r88_audit["route_horizon_blended_phase_details"]["1"]["storage_risk_term"] == 0.9
    assert r88_audit["selected_action"] == 0
