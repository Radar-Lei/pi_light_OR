...
from lock_v15_r6_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r6_terminal_flush_controller_is_registered() -> None:
...
def test_r6_training_protocol_uses_fresh_seeds_and_is_not_claim_ready() -> None:
    ...
    assert set(split["seeds"]).isdisjoint(R4_TRAINING_SEEDS)
    assert set(split["seeds"]).isdisjoint(R5_TRAINING_SEEDS)
    assert protocol["claim_scope"]["closed_loop_superiority_claim_allowed"] is False


def two_phase_state() -> tuple[dict[str, list[str]], dict[str, list[tuple[str, str]]], dict[str, float], dict[str, float], dict[str, object], dict[str, dict[str, float]]]:
    ...
    return phase_states, movements, queues, capacities, state, dual_state


def test_r6_uses_dynamic_action_before_terminal_flush_and_double_action_after() -> None:
    phase_states, movements, queues, capacities, state, dual_state = two_phase_state()
    ...
    assert early["selected_action"] != early["finite_storage_double_action"]
    assert early["terminal_completion_fallback_used"] is False
    assert late["selected_action"] == late["finite_storage_double_action"]
    assert late["terminal_completion_fallback_used"] is True
