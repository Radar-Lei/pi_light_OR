...
from lock_v15_r94_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r94_reactivated_dual_supported_low_total_margin_cap_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_supported_low_total_margin_cap"] == 0.15
    assert protocol["controller_params"]["route_horizon_supported_low_total_total_ceiling"] == 1.0
    assert protocol["controller_params"]["route_horizon_supported_low_total_pressure_ceiling"] == 0.0


def test_r94_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r94_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r94_caps_very_low_total_thin_win_even_without_large_raw_gap(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda phase_idx, *args, **kwargs: {
            "score": 2.5 if phase_idx == 0 else 3.0,
            "components": {
                "finishable_term": 2.5 if phase_idx == 0 else 3.0,
    ...
    monkeypatch.setitem(
        DYNAMIC_V1_5_R94_REACTIVATED_DUAL_SUPPORTED_LOW_TOTAL_MARGIN_CAP_PARAMS,
        "route_horizon_supported_low_total_margin_cap",
        2.2,
    )
    ...
    r94_audit = select_finite_storage_action_with_audit(
        "J0",
        0,
        phase_states,
        movements,
    ...
    assert r94_audit["route_horizon_blended_phase_details"]["1"]["supported_low_total_margin_capped"] == 1.0
    assert r94_audit["route_horizon_blended_phase_details"]["1"]["supported_low_total_total_ceiling"] == 1.0
    assert r94_audit["selected_action"] == 0


def test_r94_skips_cap_when_low_total_candidate_is_above_strict_total_ceiling(monkeypatch) -> None:
    phase_states, movements, queues, capacities, state, dual_state, route_completion_state = base_inputs()
    ...
    assert "supported_low_total_margin_capped" not in r94_audit["route_horizon_blended_phase_details"]["1"]
    assert r94_audit["selected_action"] == 1
