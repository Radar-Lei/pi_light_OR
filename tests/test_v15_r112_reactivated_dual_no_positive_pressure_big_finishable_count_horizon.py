...
from lock_v15_r112_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r112_reactivated_dual_no_positive_pressure_big_finishable_count_horizon_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_no_positive_pressure_big_finishable_count_power"] == 0.5


def test_r112_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r112_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r112_normalizes_only_when_multiple_big_finishable_movements_coexist() -> None:
    states = ["GGG"]
    movements = [
        ("A", "B"),
        ("C", "D"),
    ...
    r111_like = phase_route_horizon_completion_decomposition(
        0,
        states,
        movements,
        capacities,
    ...
    r112 = phase_route_horizon_completion_decomposition(
        0,
        states,
        movements,
        capacities,
    ...
    r111_details = {detail["movement"]: detail for detail in r111_like["movement_details"]}
    r112_details = {detail["movement"]: detail for detail in r112["movement_details"]}

    assert r112_details["A->B"]["effective_big_finishable_count_scale"] == math.sqrt(2.0)
    assert r112_details["E->F"]["effective_big_finishable_count_scale"] == math.sqrt(2.0)
    assert r112_details["C->D"]["effective_big_finishable_count_scale"] == 1.0
    assert r112_details["A->B"]["finishable_term"] < r111_details["A->B"]["finishable_term"]
    assert r112_details["E->F"]["finishable_term"] < r111_details["E->F"]["finishable_term"]
    assert math.isclose(r112_details["C->D"]["finishable_term"], r111_details["C->D"]["finishable_term"], rel_tol=1e-9)


def test_r112_skips_big_finishable_count_normalization_when_only_one_big_movement_exists() -> None:
    states = ["GGG"]
    movements = [
        ("A", "B"),
        ("C", "D"),
    ...
    assert details["A->B"]["effective_big_finishable_count_scale"] == 1.0
    assert details["C->D"]["effective_big_finishable_count_scale"] == 1.0
    assert details["E->F"]["effective_big_finishable_count_scale"] == 1.0
