...
from lock_v15_r32_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def weak_completion_inputs() -> tuple:
...
def test_r32_preterminal_double_guard_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_double_pressure_envelope"] == 1.0
    assert protocol["controller_params"]["route_horizon_double_pressure_envelope_start_fraction"] == 0.82


def test_r32_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r32_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r32_double_guard_is_inactive_before_preterminal_window() -> None:
    ...
    assert audit["route_horizon_double_pressure_envelope_used"] is False


def test_r32_double_guard_reverts_weak_completion_action_in_preterminal_window() -> None:
    phase_states, movements, queues, capacities, state, dual_state, route_completion_state = weak_completion_inputs()
    ...
    assert audit["route_horizon_completion_filter_used"] is True
    assert audit["route_horizon_double_pressure_envelope_used"] is True
