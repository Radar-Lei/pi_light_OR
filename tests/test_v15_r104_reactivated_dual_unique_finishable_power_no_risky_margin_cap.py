...
from lock_v15_r103_training_protocol import CONTROLLER_ID as R103_CONTROLLER_ID  # noqa: E402
from lock_v15_r104_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r104_reactivated_dual_unique_finishable_power_no_risky_margin_cap_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_no_risky_movement_score_advantage_floor"] == 1.0


def test_r104_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r104_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r104_caps_no_risky_thin_wins_when_core_horizon_score_is_clearly_better(monkeypatch) -> None:
    ...
    def fake_horizon_decomp(phase_idx: int, *args, **kwargs) -> dict[str, object]:
        if phase_idx == 0:
            return {
                ...
                "movement_details": [{"effective_time_weight_power": 1.0}],
            }
        return {
            "score": 1.5,
    ...
    r103_audit = select_finite_storage_action_with_audit(
        "J0",
        0,
        phase_states,
        movements,
    ...
    assert r103_audit["selected_action"] == 1
    assert r104_audit["route_horizon_blended_phase_details"]["1"]["no_risky_movement_margin_capped"] == 1.0
    assert r104_audit["route_horizon_blended_phase_details"]["1"]["route_horizon_score_gap_to_best_core"] == 1.5
    assert r104_audit["selected_action"] == 0


def test_r104_skips_cap_when_selected_phase_has_risky_movement_time_weight(monkeypatch) -> None:
    phase_states, movements, queues, capacities, state, dual_state, route_completion_state = base_inputs()
    ...
    def fake_horizon_decomp(phase_idx: int, *args, **kwargs) -> dict[str, object]:
        ...
        return {
            "score": 1.5,
            "components": {"finishable_term": 1.5, "downstream_penalty": 0.0, "local_pressure_penalty": 0.0},
            "movement_details": [{"effective_time_weight_power": 2.0}],
        }
    ...
    assert "no_risky_movement_margin_capped" not in r104_audit["route_horizon_blended_phase_details"]["1"]
    assert r104_audit["route_horizon_capped_phases"] == []
    assert r104_audit["selected_action"] == 1
