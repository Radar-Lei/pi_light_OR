...
from lock_v15_r90_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r90_reactivated_dual_supported_margin_cap_controller_is_registered() -> None:
...
def test_r90_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r90_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r90_caps_only_when_core_baseline_has_horizon_support(monkeypatch) -> None:
    ...
    r90_audit = select_finite_storage_action_with_audit(
        "J0",
        0,
        phase_states,
        movements,
    ...
    assert r79_audit["route_horizon_blended_phase_details"]["1"]["blended_score"] == 2.0
    assert r90_audit["route_horizon_blended_phase_details"]["1"]["thin_margin_low_signal_capped"] == 1.0
    assert r90_audit["selected_action"] == 0


def test_r90_skips_cap_when_core_baseline_lacks_horizon_support(monkeypatch) -> None:
    phase_states, movements, queues, capacities, state, dual_state, route_completion_state = base_inputs()
    ...
    assert r90_audit["route_horizon_blended_phase_details"]["0"]["horizon_term"] == 0.0
    assert r90_audit["route_horizon_blended_phase_details"]["1"]["blended_score"] == 3.0
    assert r90_audit["route_horizon_capped_phases"] == []
    assert r90_audit["selected_action"] == 1
