...
from lock_v15_r29_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r29_core_baseline_envelope_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_core_baseline_envelope"] == 1.0
    assert protocol["controller_params"]["route_horizon_core_baseline_margin"] == 0.10


def test_r29_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r29_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r29_reverts_to_core_baseline_when_completion_advantage_is_weak() -> None:
    ...
    assert audit["route_horizon_core_baseline_envelope_used"] is True
...
def test_r29_keeps_horizon_action_when_completion_advantage_is_strong() -> None:
    phase_states, movements, queues, capacities, state, dual_state, route_completion_state = base_inputs()
    ...
    assert audit["route_horizon_completion_filter_used"] is True
    assert audit["route_horizon_core_baseline_envelope_used"] is False
