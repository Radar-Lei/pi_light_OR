...
from lock_v15_r80_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r80_reactivated_dual_negative_total_raw_consensus_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_negative_total_raw_consensus_guard"] == 1.0
    assert protocol["controller_params"]["route_horizon_negative_total_raw_consensus_occupancy_threshold"] == 0.90
    assert protocol["controller_params"]["route_horizon_negative_total_raw_consensus_total_ceiling"] == -5.0


def test_r80_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r80_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r80_negative_total_raw_consensus_guard_only_catches_non_completion_overrides(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: {"max_occupancy_ratio": 1.0, "min_residual_ratio": 0.0},
    )
    ...
    assert guarded_audit["route_horizon_completion_filter_used"] is False
    assert guarded_audit["route_horizon_negative_total_raw_consensus_guard_used"] is True
