...
from lock_v15_r15_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r15_horizon_model_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_completion_filter"] == 1.0


def test_r15_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r15_training_only" for row in spec)
...
def test_r15_prioritizes_finishable_route_demand() -> None:
    ...
    assert audit["route_horizon_completion_phase_scores"]["1"] > audit["route_horizon_completion_phase_scores"]["0"]
