...
from lock_v15_r44_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r44_loose_staged_pressure_safe_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_pressure_safe_capacity_tolerance"] == 0.50
    assert "route_horizon_deadline_time_urgency" not in protocol["controller_params"]


def test_r44_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r44_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r44_pressure_safe_guard_is_looser_than_r43() -> None:
    protocol = build_training_protocol()
    params = protocol["controller_params"]

    assert params["route_horizon_pressure_safe_min_completion_gain"] == 0.05
    assert params["route_horizon_pressure_safe_capacity_tolerance"] > 0.20
    assert params["route_horizon_pressure_safe_double_tolerance"] > 0.20


def test_r44_still_blocks_completion_neutral_core_weak_horizon_action() -> None:
