...
from lock_v15_r17_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r17_horizon_capacity_terminal_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["terminal_flush_action"] == "capacity_aware"


def test_r17_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r17_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r17_terminal_lock_uses_capacity_aware_action() -> None:
