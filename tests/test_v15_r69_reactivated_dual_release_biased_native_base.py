...
from lock_v15_r69_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r69_reactivated_dual_release_biased_native_base_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["release_threshold"] == 0.65
    assert protocol["controller_params"]["release_step_size"] == 0.70
    assert protocol["controller_params"]["alpha_release"] == 0.90
    assert protocol["controller_params"]["beta_storage"] == 0.16
    assert protocol["controller_params"]["gamma_cascade"] == 0.10


def test_r69_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r69_training_only" for row in spec)
