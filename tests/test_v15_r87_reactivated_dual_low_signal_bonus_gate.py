...
from lock_v15_r79_training_protocol import CONTROLLER_ID as R79_CONTROLLER_ID  # noqa: E402
from lock_v15_r87_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r87_reactivated_dual_low_signal_bonus_gate_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_low_signal_bonus_gate"] == 1.0
    assert protocol["controller_params"]["route_horizon_low_signal_bonus_total_ceiling"] == 2.0
    assert protocol["controller_params"]["route_horizon_low_signal_bonus_pressure_ceiling"] == 0.0


def test_r87_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r87_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r87_low_signal_bonus_gate_strips_weak_completion_bonus_without_new_guard(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda phase_idx, *args, **kwargs: {
            "score": 10.0 if phase_idx == 0 else 9.0,
            "components": {
                "finishable_term": 10.0 if phase_idx == 0 else 9.0,
    ...
    r79_audit = select_finite_storage_action_with_audit(
        "J0",
        0,
        phase_states,
        movements,
    ...
    r87_audit = select_finite_storage_action_with_audit(
        "J0",
        0,
        phase_states,
        movements,
    ...
    assert r79_audit["route_horizon_completion_filter_used"] is True
    assert r79_audit["selected_action"] == 1
    assert r79_audit["route_horizon_blended_phase_details"]["1"]["blended_score"] > r79_audit["route_horizon_blended_phase_details"]["0"]["blended_score"]

    assert r87_audit["route_horizon_completion_filter_used"] is True
    assert r87_audit["route_horizon_negative_total_completion_conflict_guard_used"] is False
    assert r87_audit["route_horizon_blended_phase_details"]["1"]["low_signal_bonus_blocked"] == 1.0
    assert r87_audit["route_horizon_blended_phase_details"]["1"]["release_term"] == 0.0
    assert r87_audit["route_horizon_blended_phase_details"]["1"]["storage_risk_term"] == 0.0
    assert r87_audit["route_horizon_blended_phase_details"]["1"]["negative_total_term"] == 0.0
    assert r87_audit["selected_action"] == 0
