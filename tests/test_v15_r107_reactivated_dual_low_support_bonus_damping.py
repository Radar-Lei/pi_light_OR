...
from lock_v15_r107_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r107_reactivated_dual_low_support_bonus_damping_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_low_support_bonus_damping_scale"] == 0.9


def test_r107_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r107_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r107_damps_release_and_storage_risk_on_low_support_thin_win(monkeypatch) -> None:
    ...
    r107_audit = select_finite_storage_action_with_audit(
        "J0",
        0,
        phase_states,
        movements,
    ...
    detail = r107_audit["route_horizon_blended_phase_details"]["1"]
    assert detail["low_support_bonus_damped"] == 1.0
    assert detail["release_term"] == 0.9
    assert detail["storage_risk_term"] == 0.9
    assert r107_audit["selected_action"] == 0
