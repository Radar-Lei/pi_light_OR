...
from lock_v15_r83_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r83_reactivated_dual_low_total_consensus_completion_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_low_total_consensus_completion_guard"] == 1.0
    assert protocol["controller_params"]["route_horizon_low_total_consensus_completion_end_fraction"] == 0.15
    assert protocol["controller_params"]["route_horizon_low_total_consensus_completion_total_ceiling"] == 2.0


def test_r83_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r83_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r83_low_total_consensus_completion_guard_catches_weak_early_consensus_override(monkeypatch) -> None:
    ...
    assert audit["route_horizon_low_total_consensus_completion_guard_used"] is True
    assert audit["selected_action"] == 0


def test_r83_low_total_consensus_completion_guard_preserves_strong_positive_pressure_override(monkeypatch) -> None:
    phase_states = {"J0": ["Gr", "rG"]}
    movements = {"J0": [("up_a", "down_a"), ("up_b", "down_b")]}
    queues = {"up_a": 10.0, "down_a": 0.0, "up_b": 3.0, "down_b": 0.0}
    capacities = {edge: 10.0 for edge in queues}
    ...
    assert audit["route_horizon_completion_filter_used"] is True
    assert audit["route_horizon_low_total_consensus_completion_guard_used"] is False
