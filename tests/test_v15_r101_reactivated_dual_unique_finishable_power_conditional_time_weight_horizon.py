...
from lock_v15_r101_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r101_reactivated_dual_unique_finishable_power_conditional_time_weight_horizon_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_conditional_phase_time_weight_power"] == 2.0


def test_r101_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r101_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r101_conditional_time_weight_only_sharpens_no_positive_pressure_low_residual_phase() -> None:
    states = ["GG"]
    movements = [
        ("A", "B"),
        ("C", "D"),
    ...
    no_positive = phase_route_horizon_completion_decomposition(
        0,
        states,
        movements,
        capacities,
    ...
    with_positive = phase_route_horizon_completion_decomposition(
        0,
        states,
        movements,
        capacities,
        ...
        route_completion_state,
        params,
        queues={"A": 2.0, "B": 0.0, "C": 1.0, "D": 1.0},
    )

    no_positive_details = {detail["movement"]: detail for detail in no_positive["movement_details"]}
    with_positive_details = {detail["movement"]: detail for detail in with_positive["movement_details"]}

    assert no_positive_details["A->B"]["effective_time_weight_power"] == 2.0
    assert no_positive_details["C->D"]["effective_time_weight_power"] == 2.0
    assert with_positive_details["A->B"]["effective_time_weight_power"] == 1.0
    assert with_positive_details["C->D"]["effective_time_weight_power"] == 1.0
    assert no_positive_details["C->D"]["finishable_term"] < with_positive_details["C->D"]["finishable_term"]
