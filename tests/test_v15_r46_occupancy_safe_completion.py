...
from lock_v15_r46_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r46_occupancy_safe_completion_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["completion_safety_start_fraction"] == 0.70
    assert protocol["controller_params"]["completion_safety_occupancy_threshold"] == 0.85
    assert protocol["controller_params"]["completion_safety_residual_threshold"] == 0.20


def test_r46_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r46_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r46_completion_safety_veto_stays_inactive_below_occupancy_gate() -> None:
    phase_states, movements, queues, capacities, state, dual_state, route_completion_state = _risk_case_inputs(0.79)
...
def test_r46_completion_safety_veto_reverts_preterminal_risk_under_high_occupancy() -> None:
    phase_states, movements, queues, capacities, state, dual_state, route_completion_state = _risk_case_inputs(0.95)
    ...
    assert audit["route_horizon_pressure_safe_guard_used"] is True
    assert audit["completion_safety_veto_used"] is True
