...
from lock_v15_r53_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r53_post_veto_double_safe_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["post_completion_veto_double_conflict_occupancy_threshold"] == 0.90


def test_r53_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r53_training_only" for row in spec)
