...
from lock_v15_r65_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r65_reactivated_dual_native_capacity_score_rescue_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_native_capacity_score_rescue_guard"] == 1.0
    assert protocol["controller_params"]["route_horizon_native_capacity_score_rescue_min_total_gap"] == 5.0
    assert protocol["controller_params"]["route_horizon_native_capacity_score_rescue_selected_total_ceiling"] == -40.0


def test_r65_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r65_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r65_native_capacity_score_rescue_reverts_deeply_negative_completion_filter_conflict(monkeypatch) -> None:
    phase_states, movements, queues, capacities, state, dual_state, route_completion_state = base_inputs()
    for edge in ("down_a", "down_b"):
        state["spillback_blocking"][edge]["occupancy_ratio"] = 1.35
        state["residual_receiving_capacity"][edge] = 0.0
    ...
    def fake_phase_score(
        ...
        if controller in {"max_pressure", "capacity_aware_pressure"}:
            return 20.0 if phase_idx == 1 else 0.0
        if controller == "finite_storage_double_pressure":
            return 20.0 if phase_idx == 0 else 0.0
    ...
    monkeypatch.setattr(sumo, "phase_route_horizon_completion_score", lambda *args, **kwargs: 100.0 if args[0] == 0 else 10.0)
    ...
    assert audit["route_horizon_native_capacity_score_rescue_guard_used"] is True
    assert audit["finite_storage_double_action"] == 0
    assert audit["pressure_action"] == audit["capacity_aware_action"] == 1
    assert audit["selected_action"] == audit["capacity_aware_action"] == 1
