...
from lock_v15_r71_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r71_reactivated_dual_local_descendant_release_native_base_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["release_local_child_slack_floor"] == 0.05
    assert protocol["controller_params"]["release_descendant_support_weight"] == 0.35
    assert protocol["controller_params"]["release_descendant_occupancy_threshold"] == 0.90


def test_r71_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r71_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r71_release_price_stays_off_without_local_child_slack() -> None:
    movements = {
        "J0": [("up_a", "mid_a")],
        "J1": [("mid_a", "down_a")],
    }
    ...
    vehicle_counts = {"up_a": 19.0, "mid_a": 10.0, "down_a": 2.0}
...
def test_r71_release_price_activates_with_local_and_descendant_slack_support() -> None:
    movements = {
        "J0": [("up_a", "mid_a")],
        "J1": [("mid_a", "down_a")],
    }
    capacities = {"up_a": 20.0, "mid_a": 10.0, "down_a": 10.0}
    queues = {"up_a": 0.0, "mid_a": 0.0, "down_a": 0.0}
    vehicle_counts = {"up_a": 19.0, "mid_a": 9.0, "down_a": 2.0}
    state = build_completed_finite_storage_state(
        queues,
        capacities,
        vehicle_counts=vehicle_counts,
