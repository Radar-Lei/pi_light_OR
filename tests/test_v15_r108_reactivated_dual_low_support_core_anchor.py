...
from lock_v15_r108_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r108_reactivated_dual_low_support_core_anchor_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_low_support_core_anchor_bonus"] == 0.11


def test_r108_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r108_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r108_adds_core_anchor_on_low_support_thin_win(monkeypatch) -> None:
    ...
    def fake_horizon_decomp(phase_idx: int, *args, **kwargs) -> dict[str, object]:
        if phase_idx == 0:
            return {
                "score": 1.3,
                "components": {"finishable_term": 1.3, "downstream_penalty": 0.0, "local_pressure_penalty": 0.0},
                "movement_details": [{"effective_time_weight_power": 1.0, "local_pressure": 0.0}],
            }
        return {
            "score": 0.2,
            "components": {"finishable_term": 0.2, "downstream_penalty": 0.0, "local_pressure_penalty": 0.0},
            "movement_details": [{"effective_time_weight_power": 1.0, "local_pressure": 0.0}],
        }
    ...
    r108_audit = select_finite_storage_action_with_audit(
        "J0",
        0,
        phase_states,
        movements,
    ...
    core_detail = r108_audit["route_horizon_blended_phase_details"]["0"]
    assert core_detail["low_support_core_anchor_bonus"] == 0.11
    assert r108_audit["selected_action"] == 0
