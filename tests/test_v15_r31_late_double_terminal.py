...
from lock_v15_r31_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r31_late_double_terminal_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"].get("terminal_flush_action", "finite_storage_double") == "finite_storage_double"


def test_r31_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r31_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r31_does_not_lock_terminal_before_late_double_threshold() -> None:
...
def test_r31_locks_to_finite_storage_double_after_terminal_threshold() -> None:
    phase_states, movements, queues, capacities, state, dual_state, route_completion_state = base_inputs()
