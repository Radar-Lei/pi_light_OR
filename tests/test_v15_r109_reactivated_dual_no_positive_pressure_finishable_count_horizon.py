...
from lock_v15_r109_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r109_reactivated_dual_no_positive_pressure_finishable_count_horizon_controller_is_registered() -> None:
...
def test_r109_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r109_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r109_normalizes_finishable_credit_by_positive_movement_count_only_in_supported_phase() -> None:
    states = ["GGG"]
    movements = [
        ("A", "B"),
        ("C", "D"),
    ...
    assert base["components"]["finishable_term"] == 3.2
    assert math.isclose(
        normalized["components"]["finishable_term"],
        3.2 / math.sqrt(3.0),
        rel_tol=1e-9,
    )
    assert normalized_details["A->B"]["effective_finishable_count_scale"] == math.sqrt(3.0)
    assert normalized_details["C->D"]["effective_finishable_count_scale"] == math.sqrt(3.0)
    assert normalized_details["E->F"]["effective_finishable_count_scale"] == math.sqrt(3.0)
    assert base_details["A->B"]["finishable_term"] > normalized_details["A->B"]["finishable_term"]


def test_r109_skips_count_normalization_when_phase_has_positive_local_pressure() -> None:
    states = ["GG"]
    movements = [
        ("A", "B"),
        ("C", "D"),
