...
from lock_v15_r81_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r81_reactivated_dual_severe_negative_total_raw_consensus_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_negative_total_raw_consensus_total_ceiling"] == -20.0


def test_r81_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r81_training_only" for row in spec)
