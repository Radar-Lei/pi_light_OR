...
from lock_v15_r67_training_protocol import CONTROLLER_ID, build_training_protocol  # noqa: E402
...
def test_r67_reactivated_dual_native_negative_total_penalty_controller_is_registered() -> None:
    ...
    assert protocol["controller_params"]["route_horizon_native_negative_total_penalty"] == 0.04


def test_r67_training_spec_is_training_only_and_fresh() -> None:
    protocol = build_training_protocol()
    spec = build_training_spec(protocol)

    assert len(spec) == protocol["training_split"]["expected_row_count"]
    assert all(row["evidence_role"] == "v1_5_r67_training_only" for row in spec)
    assert all(row["gate_c_eligible"] is False for row in spec)
    assert CONTROLLER_ID in {row["controller"] for row in spec}


def test_r67_route_horizon_penalty_discourages_deeply_negative_native_total(monkeypatch) -> None:
    ...
    def fake_dynamic_phase_decomposition(
        ...
        *,
        ...
        total = -50.0 if phase_index == 0 else 0.0
        component_totals = {
            "cascade_price": 0.0,
