#!/usr/bin/env python3
"""Execute or resume the v1.7 CFS-PD-MPC training protocol.
...
Supports:
  ...
  --screen-only      Execute only the 12-row screen subset (3 scenarios x 1 seed
                     x 1 demand x 4 controllers) for early smoke testing.
...
DEFAULT_OUT = "experiments/dual_sensitivity/v1_7_training_execution.json"
DEFAULT_PROGRESS = "experiments/dual_sensitivity/v1_7_training_execution.progress.json"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Execute v1.7 CFS-PD-MPC training protocol")
    ...
    parser.add_argument("--screen-only", action="store_true",
                        help="Execute only the 12-row screen subset for smoke testing")
...
def build_training_spec(protocol: dict[str, Any]) -> list[dict[str, Any]]:
                    ...
                    spec.append(
                        {
                            ...
                            "evidence_role": "v1_7_training_only",
    ...
    if len(spec) != expected:
        raise ValueError(
            f"v1.7 training spec row count {len(spec)} does not match expected_row_count={expected}"
        )
    return spec


def build_screen_spec(protocol: dict[str, Any]) -> list[dict[str, Any]]:
    """Build the 12-row screen subset from the training protocol.

    Screen: 3 scenarios x 1 seed x 1 demand x 4 controllers = 12 rows.
    """
    split = protocol["training_split"]
            ...
            for seed in screen_seeds:
                for controller in screen_controllers:
                    spec.append(
                        {
                            "profile": "screen",
                            "evidence_role": "v1_7_screen_only",
                            "gate_c_eligible": False,
                            "network": "arterial",
                            "scenario_tag": str(scenario_tag),
                            "controller": str(controller),
...
def write_training_execution(
    *,
    ...
    payload = {
        "experiment": "v1_7_training_execution",
