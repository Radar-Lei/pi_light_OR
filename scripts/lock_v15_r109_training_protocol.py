#!/usr/bin/env python3
"""Lock a post-r108 v1.5 reactivated-dual no-positive-pressure finishable-count-horizon training protocol."""
...
DEFAULT_OUT = "experiments/dual_sensitivity/v1_5_r109_training_protocol.json"
CONTROLLER_ID = "finite_storage_dynamic_primal_dual_v1_5_r109_reactivated_dual_no_positive_pressure_finishable_count_horizon"
REQUIREMENTS_COVERED = ["V15-R109-CTRL-01", "V15-CLAIM-01"]
TRAINING_SEEDS = [20261601 + idx for idx in range(6)]
...
def build_training_protocol() -> dict[str, Any]:
    ...
    if overlap:
        raise ValueError(f"r109 training seeds overlap excluded seeds: {overlap}")
    ...
    core: dict[str, Any] = {
        ...
        "controller_params": DYNAMIC_V1_5_R109_REACTIVATED_DUAL_NO_POSITIVE_PRESSURE_FINISHABLE_COUNT_HORIZON_PARAMS,
        ...
        "selection_rule": {
            ...
            "eligible_for_selection_only_if": [
                ...
                "route-horizon no-positive-local-pressure phases with materially pressured downstream routes normalize finishable credit by positive finishable-movement count",
    ...
    return {
        "experiment": "v1_5_r109_training_protocol",
        "status": "TRAINING_LOCKED",
        "generated_by": "scripts/lock_v15_r109_training_protocol.py",
        "requirements_covered": REQUIREMENTS_COVERED,
        **core,
        "protocol_fingerprint": base.stable_fingerprint(core),
        "artifact_plan": {
            "training_protocol": DEFAULT_OUT,
            "training_execution": "experiments/dual_sensitivity/v1_5_r109_training_execution.json",
            "training_selection": "experiments/dual_sensitivity/v1_5_r109_training_selection.json",
        },
