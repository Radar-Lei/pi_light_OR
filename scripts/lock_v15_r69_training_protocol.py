#!/usr/bin/env python3
"""Lock a post-r63 v1.5 reactivated-dual-release-biased-native-base training protocol."""
...
from lock_v15_r67_training_protocol import TRAINING_SEEDS as R67_TRAINING_SEEDS
...
from run_closed_loop_sumo import DYNAMIC_V1_5_R69_REACTIVATED_DUAL_RELEASE_BIASED_NATIVE_BASE_PARAMS

DEFAULT_OUT = "experiments/dual_sensitivity/v1_5_r69_training_protocol.json"
CONTROLLER_ID = "finite_storage_dynamic_primal_dual_v1_5_r69_reactivated_dual_release_biased_native_base"
REQUIREMENTS_COVERED = ["V15-R69-CTRL-01", "V15-CLAIM-01"]
TRAINING_SEEDS = [20261361 + idx for idx in range(6)]
...
def build_training_protocol() -> dict[str, Any]:
    ...
    excluded = (
        ...
        | set(R67_TRAINING_SEEDS)
        | set(R68_TRAINING_SEEDS)
    )
    overlap = sorted(set(TRAINING_SEEDS) & excluded)
    if overlap:
        raise ValueError(f"r69 training seeds overlap excluded seeds: {overlap}")
    ...
    core: dict[str, Any] = {
        ...
        "controller_params": DYNAMIC_V1_5_R69_REACTIVATED_DUAL_RELEASE_BIASED_NATIVE_BASE_PARAMS,
        ...
        "selection_rule": {
            ...
            "eligible_for_selection_only_if": [
                ...
                "native dynamic decomposition assigns stronger value to upstream release and slightly relaxes storage/cascade suppression when pressure-capacity baselines agree on the safer release action",
    ...
    return {
        "experiment": "v1_5_r69_training_protocol",
        "status": "TRAINING_LOCKED",
        "generated_by": "scripts/lock_v15_r69_training_protocol.py",
        "requirements_covered": REQUIREMENTS_COVERED,
        **core,
        "protocol_fingerprint": base.stable_fingerprint(core),
        "artifact_plan": {
            "training_protocol": DEFAULT_OUT,
            "training_execution": "experiments/dual_sensitivity/v1_5_r69_training_execution.json",
            "training_selection": "experiments/dual_sensitivity/v1_5_r69_training_selection.json",
        },
