#!/usr/bin/env python3
"""Lock a post-r103 v1.5 reactivated-dual unique-finishable-power secondary-time-weight-horizon training protocol."""
...
DEFAULT_OUT = "experiments/dual_sensitivity/v1_5_r106_training_protocol.json"
CONTROLLER_ID = "finite_storage_dynamic_primal_dual_v1_5_r106_reactivated_dual_unique_finishable_power_secondary_time_weight_horizon"
REQUIREMENTS_COVERED = ["V15-R106-CTRL-01", "V15-CLAIM-01"]
TRAINING_SEEDS = [20261583 + idx for idx in range(6)]
...
def build_training_protocol() -> dict[str, Any]:
    ...
    if overlap:
        raise ValueError(f"r106 training seeds overlap excluded seeds: {overlap}")
    ...
    core: dict[str, Any] = {
        ...
        "controller_params": DYNAMIC_V1_5_R106_REACTIVATED_DUAL_UNIQUE_FINISHABLE_POWER_SECONDARY_TIME_WEIGHT_HORIZON_PARAMS,
        ...
        "selection_rule": {
            ...
            "eligible_for_selection_only_if": [
                ...
                "route-horizon source ranking adds a gentler secondary time-normalization band for no-positive-local-pressure moderate-residual movements pointing into pressured downstreams",
    ...
    return {
        "experiment": "v1_5_r106_training_protocol",
        "status": "TRAINING_LOCKED",
        "generated_by": "scripts/lock_v15_r106_training_protocol.py",
        "requirements_covered": REQUIREMENTS_COVERED,
        **core,
        "protocol_fingerprint": base.stable_fingerprint(core),
        "artifact_plan": {
            "training_protocol": DEFAULT_OUT,
            "training_execution": "experiments/dual_sensitivity/v1_5_r106_training_execution.json",
            "training_selection": "experiments/dual_sensitivity/v1_5_r106_training_selection.json",
        },
