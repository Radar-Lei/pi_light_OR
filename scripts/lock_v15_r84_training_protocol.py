#!/usr/bin/env python3
"""Lock a post-r79 v1.5 reactivated-dual conditional release-gate training protocol."""
...
from run_closed_loop_sumo import DYNAMIC_V1_5_R84_REACTIVATED_DUAL_CONDITIONAL_RELEASE_GATE_PARAMS

DEFAULT_OUT = "experiments/dual_sensitivity/v1_5_r84_training_protocol.json"
CONTROLLER_ID = "finite_storage_dynamic_primal_dual_v1_5_r84_reactivated_dual_conditional_release_gate"
REQUIREMENTS_COVERED = ["V15-R84-CTRL-01", "V15-CLAIM-01"]
TRAINING_SEEDS = [20261451 + idx for idx in range(6)]
...
def build_training_protocol() -> dict[str, Any]:
    ...
    if overlap:
        raise ValueError(f"r84 training seeds overlap excluded seeds: {overlap}")
    ...
    core: dict[str, Any] = {
        ...
        "controller_params": DYNAMIC_V1_5_R84_REACTIVATED_DUAL_CONDITIONAL_RELEASE_GATE_PARAMS,
        ...
        "selection_rule": {
            ...
            "eligible_for_selection_only_if": [
                ...
                "conditional release gate only suppresses release bonus for low-total nonpositive-pressure completion candidates inside the early severe regime",
    ...
    return {
        "experiment": "v1_5_r84_training_protocol",
        "status": "TRAINING_LOCKED",
        "generated_by": "scripts/lock_v15_r84_training_protocol.py",
        "requirements_covered": REQUIREMENTS_COVERED,
        **core,
        "protocol_fingerprint": base.stable_fingerprint(core),
        "artifact_plan": {
            "training_protocol": DEFAULT_OUT,
            "training_execution": "experiments/dual_sensitivity/v1_5_r84_training_execution.json",
            "training_selection": "experiments/dual_sensitivity/v1_5_r84_training_selection.json",
        },
