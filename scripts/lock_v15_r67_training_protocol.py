#!/usr/bin/env python3
"""Lock a post-r63 v1.5 reactivated-dual-native-negative-total-penalty training protocol."""
...
from lock_v15_r66_training_protocol import TRAINING_SEEDS as R66_TRAINING_SEEDS
from lock_v15_r63_training_protocol import build_training_protocol as build_r63_training_protocol
from run_closed_loop_sumo import DYNAMIC_V1_5_R67_REACTIVATED_DUAL_NATIVE_NEGATIVE_TOTAL_PENALTY_PARAMS

DEFAULT_OUT = "experiments/dual_sensitivity/v1_5_r67_training_protocol.json"
CONTROLLER_ID = "finite_storage_dynamic_primal_dual_v1_5_r67_reactivated_dual_native_negative_total_penalty"
REQUIREMENTS_COVERED = ["V15-R67-CTRL-01", "V15-CLAIM-01"]
TRAINING_SEEDS = [20261349 + idx for idx in range(6)]
...
def build_training_protocol() -> dict[str, Any]:
    ...
    excluded = (
        ...
        | set(R66_TRAINING_SEEDS)
    )
    overlap = sorted(set(TRAINING_SEEDS) & excluded)
    if overlap:
        raise ValueError(f"r67 training seeds overlap excluded seeds: {overlap}")
    ...
    core: dict[str, Any] = {
        ...
        "controller_params": DYNAMIC_V1_5_R67_REACTIVATED_DUAL_NATIVE_NEGATIVE_TOTAL_PENALTY_PARAMS,
        ...
        "selection_rule": {
            ...
            "eligible_for_selection_only_if": [
                ...
                "route-horizon completion modeling softly discounts actions whose native dynamic total is already strongly negative",
    ...
    return {
        "experiment": "v1_5_r67_training_protocol",
        "status": "TRAINING_LOCKED",
        "generated_by": "scripts/lock_v15_r67_training_protocol.py",
        "requirements_covered": REQUIREMENTS_COVERED,
        **core,
        "protocol_fingerprint": base.stable_fingerprint(core),
        "artifact_plan": {
            "training_protocol": DEFAULT_OUT,
            "training_execution": "experiments/dual_sensitivity/v1_5_r67_training_execution.json",
            "training_selection": "experiments/dual_sensitivity/v1_5_r67_training_selection.json",
        },
