#!/usr/bin/env python3
"""Lock a post-r92 v1.5 reactivated-dual supported-big-finishable-sum-horizon training protocol."""
...
from lock_v15_r113_training_protocol import TRAINING_SEEDS as R113_TRAINING_SEEDS
from run_closed_loop_sumo import (
    DYNAMIC_V1_5_R114_REACTIVATED_DUAL_SUPPORTED_BIG_FINISHABLE_SUM_HORIZON_PARAMS,
)

DEFAULT_OUT = "experiments/dual_sensitivity/v1_5_r114_training_protocol.json"
CONTROLLER_ID = "finite_storage_dynamic_primal_dual_v1_5_r114_reactivated_dual_supported_big_finishable_sum_horizon"
REQUIREMENTS_COVERED = ["V15-R114-CTRL-01", "V15-CLAIM-01"]
TRAINING_SEEDS = [20261631 + idx for idx in range(6)]
...
def build_training_protocol() -> dict[str, Any]:
    ...
    excluded = (
        ...
        | set(R113_TRAINING_SEEDS)
    )
    overlap = sorted(set(TRAINING_SEEDS) & excluded)
    if overlap:
        raise ValueError(f"r114 training seeds overlap excluded seeds: {overlap}")
    ...
    core: dict[str, Any] = {
        ...
        "controller_params": DYNAMIC_V1_5_R114_REACTIVATED_DUAL_SUPPORTED_BIG_FINISHABLE_SUM_HORIZON_PARAMS,
        ...
        "selection_rule": {
            ...
            "eligible_for_selection_only_if": [
                ...
                "route-horizon big-finishable-sum normalization only applies in no-positive-local-pressure high-downstream low-residual phases",
    ...
    return {
        "experiment": "v1_5_r114_training_protocol",
        "status": "TRAINING_LOCKED",
        "generated_by": "scripts/lock_v15_r114_training_protocol.py",
        "requirements_covered": REQUIREMENTS_COVERED,
        **core,
        "protocol_fingerprint": base.stable_fingerprint(core),
        "artifact_plan": {
            "training_protocol": DEFAULT_OUT,
            "training_execution": "experiments/dual_sensitivity/v1_5_r114_training_execution.json",
            "training_selection": "experiments/dual_sensitivity/v1_5_r114_training_selection.json",
        },
