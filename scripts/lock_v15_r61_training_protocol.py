#!/usr/bin/env python3
"""Lock a post-r59 v1.5 reactivated-dual-mid-severe-early-capacity-conflict-safe training protocol."""
...
from run_closed_loop_sumo import DYNAMIC_V1_5_R61_REACTIVATED_DUAL_MID_SEVERE_EARLY_CAPACITY_CONFLICT_SAFE_PARAMS

DEFAULT_OUT = "experiments/dual_sensitivity/v1_5_r61_training_protocol.json"
CONTROLLER_ID = "finite_storage_dynamic_primal_dual_v1_5_r61_reactivated_dual_mid_severe_early_capacity_conflict_safe"
REQUIREMENTS_COVERED = ["V15-R61-CTRL-01", "V15-CLAIM-01"]
TRAINING_SEEDS = [20261307 + idx for idx in range(6)]
...
def build_training_protocol() -> dict[str, Any]:
    ...
    if overlap:
        raise ValueError(f"r61 training seeds overlap excluded seeds: {overlap}")
    ...
    core: dict[str, Any] = {
        ...
        "controller_params": DYNAMIC_V1_5_R61_REACTIVATED_DUAL_MID_SEVERE_EARLY_CAPACITY_CONFLICT_SAFE_PARAMS,
        ...
        "selection_rule": {
            ...
            "eligible_for_selection_only_if": [
                ...
                "during a narrow mid-severe early completion-filter window, pressure-capacity consensus against double is corrected to capacity only below an occupancy upper bound",
    ...
    return {
        "experiment": "v1_5_r61_training_protocol",
        "status": "TRAINING_LOCKED",
        "generated_by": "scripts/lock_v15_r61_training_protocol.py",
        "requirements_covered": REQUIREMENTS_COVERED,
        **core,
        "protocol_fingerprint": base.stable_fingerprint(core),
        "artifact_plan": {
            "training_protocol": DEFAULT_OUT,
            "training_execution": "experiments/dual_sensitivity/v1_5_r61_training_execution.json",
            "training_selection": "experiments/dual_sensitivity/v1_5_r61_training_selection.json",
        },
