#!/usr/bin/env python3
"""Lock a post-r59 v1.5 reactivated-dual-late-severe-raw-consensus-safe training protocol."""
...
from run_closed_loop_sumo import DYNAMIC_V1_5_R62_REACTIVATED_DUAL_LATE_SEVERE_RAW_CONSENSUS_SAFE_PARAMS

DEFAULT_OUT = "experiments/dual_sensitivity/v1_5_r62_training_protocol.json"
CONTROLLER_ID = "finite_storage_dynamic_primal_dual_v1_5_r62_reactivated_dual_late_severe_raw_consensus_safe"
REQUIREMENTS_COVERED = ["V15-R62-CTRL-01", "V15-CLAIM-01"]
TRAINING_SEEDS = [20261313 + idx for idx in range(6)]
...
def build_training_protocol() -> dict[str, Any]:
    ...
    if overlap:
        raise ValueError(f"r62 training seeds overlap excluded seeds: {overlap}")
    ...
    core: dict[str, Any] = {
        ...
        "controller_params": DYNAMIC_V1_5_R62_REACTIVATED_DUAL_LATE_SEVERE_RAW_CONSENSUS_SAFE_PARAMS,
        ...
        "selection_rule": {
            ...
            "eligible_for_selection_only_if": [
                ...
                "raw-consensus fallback is delayed until later, more severe occupancy so reactivated-dual actions can separate earlier while preserving the narrow post-veto safety correction",
    ...
    return {
        "experiment": "v1_5_r62_training_protocol",
        "status": "TRAINING_LOCKED",
        "generated_by": "scripts/lock_v15_r62_training_protocol.py",
        "requirements_covered": REQUIREMENTS_COVERED,
        **core,
        "protocol_fingerprint": base.stable_fingerprint(core),
        "artifact_plan": {
            "training_protocol": DEFAULT_OUT,
            "training_execution": "experiments/dual_sensitivity/v1_5_r62_training_execution.json",
            "training_selection": "experiments/dual_sensitivity/v1_5_r62_training_selection.json",
        },
