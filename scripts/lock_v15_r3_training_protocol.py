#!/usr/bin/env python3
"""Lock a post-r2 v1.5-r3 training protocol without reusing prior holdout/training seeds."""
...
from run_closed_loop_sumo import DYNAMIC_V1_5_R3_DOUBLE_RELEASE_PARAMS

DEFAULT_OUT = "experiments/dual_sensitivity/v1_5_r3_training_protocol.json"
CONTROLLER_ID = "finite_storage_dynamic_primal_dual_v1_5_r3_double_release"
REQUIREMENTS_COVERED = ["V15-R3-TRAIN-01", "V15-CLAIM-01"]
TRAINING_SCENARIOS = [
    "arterial_v1_5_storage_activation",
    "arterial_spillback_stress",
    "arterial_downstream_blockage",
]
TRAINING_SEEDS = [20260811 + idx for idx in range(6)]
TRAINING_DEMAND_MULTIPLIERS = [0.85, 1.0, 1.15]


def build_training_protocol() -> dict[str, Any]:
    excluded = set(BINDING_HOLDOUT_SEEDS) | set(R2_TRAINING_SEEDS)
    overlap = sorted(set(TRAINING_SEEDS) & excluded)
    if overlap:
        raise ValueError(f"r3 training seeds overlap excluded seeds: {overlap}")
    ...
    core = {
        ...
        "controller_params": DYNAMIC_V1_5_R3_DOUBLE_RELEASE_PARAMS,
        ...
        "supersedes_candidate": "finite_storage_dynamic_primal_dual_v1_5_r2_guarded",
        "supersession_trigger": "experiments/dual_sensitivity/v1_5_r2_training_selection.json",
    }
    payload: dict[str, Any] = {
        "experiment": "v1_5_r3_training_protocol",
        "status": "TRAINING_LOCKED",
        "generated_by": "scripts/lock_v15_r3_training_protocol.py",
        "requirements_covered": REQUIREMENTS_COVERED,
        **core,
        "protocol_fingerprint": base.stable_fingerprint(core),
        "artifact_plan": {
            "training_protocol": DEFAULT_OUT,
            "training_execution": "experiments/dual_sensitivity/v1_5_r3_training_execution.json",
            "training_selection": "experiments/dual_sensitivity/v1_5_r3_training_selection.json",
            "future_confirmatory_protocol": "experiments/dual_sensitivity/v1_5_r3_locked_protocol.json",
        },
        "claim_scope": {
            "claim_ready": False,
