#!/usr/bin/env python3
"""Lock a post-r103 v1.5 reactivated-dual low-support-core-anchor training protocol."""
...
DEFAULT_OUT = "experiments/dual_sensitivity/v1_5_r108_training_protocol.json"
CONTROLLER_ID = "finite_storage_dynamic_primal_dual_v1_5_r108_reactivated_dual_low_support_core_anchor"
REQUIREMENTS_COVERED = ["V15-R108-CTRL-01", "V15-CLAIM-01"]
TRAINING_SEEDS = [20261595 + idx for idx in range(6)]
...
def build_training_protocol() -> dict[str, Any]:
    ...
    if overlap:
        raise ValueError(f"r108 training seeds overlap excluded seeds: {overlap}")
    ...
    core: dict[str, Any] = {
        ...
        "controller_params": DYNAMIC_V1_5_R108_REACTIVATED_DUAL_LOW_SUPPORT_CORE_ANCHOR_PARAMS,
        ...
        "selection_rule": {
            ...
            "eligible_for_selection_only_if": [
                ...
                "route-horizon blend gives the shared core baseline a small anchor bonus only for no-risky no-positive-pressure thin wins with materially weaker raw horizon support",
    ...
    return {
        "experiment": "v1_5_r108_training_protocol",
        "status": "TRAINING_LOCKED",
        "generated_by": "scripts/lock_v15_r108_training_protocol.py",
        "requirements_covered": REQUIREMENTS_COVERED,
        **core,
        "protocol_fingerprint": base.stable_fingerprint(core),
        "artifact_plan": {
            "training_protocol": DEFAULT_OUT,
            "training_execution": "experiments/dual_sensitivity/v1_5_r108_training_execution.json",
            "training_selection": "experiments/dual_sensitivity/v1_5_r108_training_selection.json",
        },
