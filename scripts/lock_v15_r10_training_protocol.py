#!/usr/bin/env python3
"""Lock a post-r9 v1.5-r10 bounded-hold training protocol with fresh seeds."""
...
from run_closed_loop_sumo import DYNAMIC_V1_5_R10_BOUNDED_HOLD_PARAMS

DEFAULT_OUT = "experiments/dual_sensitivity/v1_5_r10_training_protocol.json"
CONTROLLER_ID = "finite_storage_dynamic_primal_dual_v1_5_r10_bounded_hold"
REQUIREMENTS_COVERED = ["V15-R10-TRAIN-01", "V15-CLAIM-01"]
TRAINING_SCENARIOS = ["arterial_v1_5_storage_activation", "arterial_spillback_stress", "arterial_downstream_blockage"]
TRAINING_SEEDS = [20260881 + idx for idx in range(6)]
TRAINING_DEMAND_MULTIPLIERS = [0.85, 1.0, 1.15]


def build_training_protocol() -> dict[str, Any]:
    excluded = (
        set(BINDING_HOLDOUT_SEEDS) | set(R2_TRAINING_SEEDS) | set(R3_TRAINING_SEEDS)
        | set(R4_TRAINING_SEEDS) | set(R5_TRAINING_SEEDS) | set(R6_TRAINING_SEEDS)
        | set(R7_TRAINING_SEEDS) | set(R8_TRAINING_SEEDS) | set(R9_TRAINING_SEEDS)
    )
    overlap = sorted(set(TRAINING_SEEDS) & excluded)
    if overlap:
        raise ValueError(f"r10 training seeds overlap excluded seeds: {overlap}")
    ...
    training_split["expected_row_count"] = len(training_split["scenarios"]) * len(training_split["seeds"]) * len(training_split["demand_multipliers"]) * len(training_split["controllers"])
    core = {
        "controller_id": CONTROLLER_ID,
        "required_baselines": baselines,
        "controller_params": DYNAMIC_V1_5_R10_BOUNDED_HOLD_PARAMS,
        ...
        "selection_rule": {
            ...
            "eligible_for_selection_only_if": [
                ...
                "dynamic hold overrides are bounded by max_dynamic_hold_time",
            ],
            "not_confirmatory_evidence": True,
        },
        "supersedes_candidate": "finite_storage_dynamic_primal_dual_v1_5_r9_hold_only",
        "supersession_trigger": "experiments/dual_sensitivity/v1_5_r9_training_selection.json",
    }
    return {
        "experiment": "v1_5_r10_training_protocol",
        "status": "TRAINING_LOCKED",
        "generated_by": "scripts/lock_v15_r10_training_protocol.py",
        "requirements_covered": REQUIREMENTS_COVERED,
        **core,
        "protocol_fingerprint": base.stable_fingerprint(core),
        "artifact_plan": {
            "training_protocol": DEFAULT_OUT,
            "training_execution": "experiments/dual_sensitivity/v1_5_r10_training_execution.json",
            "training_selection": "experiments/dual_sensitivity/v1_5_r10_training_selection.json",
        },
...
def main() -> None:
    ...
    print(json.dumps({"status": payload["status"], "out": str(out), "fingerprint": payload["protocol_fingerprint"], "expected_rows": payload["training_split"]["expected_row_count"]}, indent=2))
