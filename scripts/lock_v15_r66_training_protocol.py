#!/usr/bin/env python3
"""Lock a post-r63 v1.5 reactivated-dual-native-horizon-score-blend training protocol."""
...
from lock_v15_r64_training_protocol import TRAINING_SEEDS as R64_TRAINING_SEEDS
from lock_v15_r65_training_protocol import TRAINING_SEEDS as R65_TRAINING_SEEDS
from lock_v15_r63_training_protocol import build_training_protocol as build_r63_training_protocol
from run_closed_loop_sumo import DYNAMIC_V1_5_R66_REACTIVATED_DUAL_NATIVE_HORIZON_SCORE_BLEND_PARAMS

DEFAULT_OUT = "experiments/dual_sensitivity/v1_5_r66_training_protocol.json"
CONTROLLER_ID = "finite_storage_dynamic_primal_dual_v1_5_r66_reactivated_dual_native_horizon_score_blend"
REQUIREMENTS_COVERED = ["V15-R66-CTRL-01", "V15-CLAIM-01"]
TRAINING_SEEDS = [20261343 + idx for idx in range(6)]
...
def build_training_protocol() -> dict[str, Any]:
    ...
    excluded = (
        ...
        | set(R64_TRAINING_SEEDS)
        | set(R65_TRAINING_SEEDS)
    )
    overlap = sorted(set(TRAINING_SEEDS) & excluded)
    if overlap:
        raise ValueError(f"r66 training seeds overlap excluded seeds: {overlap}")
    ...
    core: dict[str, Any] = {
        ...
        "controller_params": DYNAMIC_V1_5_R66_REACTIVATED_DUAL_NATIVE_HORIZON_SCORE_BLEND_PARAMS,
        ...
        "selection_rule": {
            ...
            "eligible_for_selection_only_if": [
                ...
                "route-horizon completion modeling prefers native dynamic finite-storage scores over the double-pressure surrogate when the two disagree",
    ...
    return {
        "experiment": "v1_5_r66_training_protocol",
        "status": "TRAINING_LOCKED",
        "generated_by": "scripts/lock_v15_r66_training_protocol.py",
        "requirements_covered": REQUIREMENTS_COVERED,
        **core,
        "protocol_fingerprint": base.stable_fingerprint(core),
        "artifact_plan": {
            "training_protocol": DEFAULT_OUT,
            "training_execution": "experiments/dual_sensitivity/v1_5_r66_training_execution.json",
            "training_selection": "experiments/dual_sensitivity/v1_5_r66_training_selection.json",
        },
