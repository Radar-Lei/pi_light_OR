...
1. **Should strict Phase 12 mode require Phase 10 `SMOKE_ONLY` to become `PASSED`?** [VERIFIED: Phase 10 artifact currently `SMOKE_ONLY`]
   - What we know: Phase 10 is capability/smoke evidence, not dominance evidence. [VERIFIED: CONTEXT.md + artifact probe]
   - What's unclear: Whether Phase 12 strict release should treat Phase 10 `SMOKE_ONLY` as acceptable context or a non-PASSED upstream status. [VERIFIED: CONTEXT.md does not define exact strict policy for Phase 10]
   - Recommendation: Treat Phase 10 `SMOKE_ONLY` as acceptable only for `baseline_stress_capability_context`, never for superiority templates. [VERIFIED: CONTEXT.md]

2. **Should Markdown output be generated by default?** [VERIFIED: CONTEXT.md allows Markdown summaries]
   - What we know: JSON is authoritative and Markdown is human-readable only. [VERIFIED: CONTEXT.md]
   - What's unclear: Whether the planner wants a single summary Markdown or only JSON/CSV. [ASSUMED]
   - Recommendation: Generate one clearly labeled audit summary Markdown and ensure claim audit scans it; do not create manuscript prose. [VERIFIED: CONTEXT.md]

3. **Should full 2160-row Phase 11 execution be callable from Phase 12?** [VERIFIED: Phase 11 expected row count]
   - What we know: Actual execution is deferred/opt-in and Phase 12 should package commands/status, not fake completion. [VERIFIED: CONTEXT.md]
   - What's unclear: Whether the plan should include a manual checkpoint for launching the expensive opt-in command. [ASSUMED]
   - Recommendation: Include the command in `phase12_reproduction_manifest.json`; do not run it during default Phase 12 validation. [VERIFIED: CONTEXT.md]
